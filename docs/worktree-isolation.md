...
worktree create 成功后，isolation 信息写入 `gate-status.json` 的 `isolation` 字段：
...
```json
{
  ...
  "isolation": {
    "<change-name>": {
      "status": "verified",
      "mode": "git-worktree",
      "path": ".sillyspec/.runtime/worktrees/xxx",
...
| status | 含义 |
|--------|------|
| verified | 有效隔离环境（git-worktree 或 native-worktree） |
| degraded | 降级为 in-place baseline protection |
| blocked | 隔离失败（submodule / gitignore 未配置） |
| pending | 尚未执行隔离检查 |
